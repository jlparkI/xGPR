...
//Generates the RBF features but using the simplex modification (Reid
//et al. 2023). This single kernel loops over 1)
//the number of repeats then inside that loop 2) the three diagonal
//matrix multiplications and fast Hadamard transforms before
//applying 3) simplex projection and 4) diagonal matmul
//before activation function.
template <typename T>
__global__ void rbfFeatureGenSimplexKernel(const T origData[], T cArray[],
        double *outputArray, const T chiArr[], const int8_t *radem,
        int paddedBufferSize, int log2N, int numFreqs, int inputElementsPerRow,
        int nRepeats, int rademShape2, T normConstant,
        double scalingConstant){
...
//Generates the RBF features with gradient, using the simplex modification
//from Reid et al. 2023. This single kernel loops over 1)
//the number of repeats then inside that loop 2) the three diagonal
//matrix multiplications and fast Hadamard transforms before
//applying 3) simplex projection before 4) diagonal matmul before activation
//function. The only difference from rbfFeatureGenSimplexKernel is that the
//gradient is also calculated.
template <typename T>
__global__ void rbfFeatureGradSimplexKernel(const T origData[], T cArray[],
        double *outputArray, const T chiArr[], const int8_t *radem,
        int paddedBufferSize, int log2N, int numFreqs, int inputElementsPerRow,
        int nRepeats, int rademShape2, T normConstant,
        double scalingConstant, double *gradient, T sigma){
