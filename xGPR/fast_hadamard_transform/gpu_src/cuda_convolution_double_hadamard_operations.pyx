...
def doubleGpuGraphConv1dTransform(reshapedX, radem, Z, S,
    ...
    of a graph, using an array of doubles.
    ...
    if mode == "gradient":
        gradient = cp.zeros((Z.shape[0], Z.shape[1], 1), dtype=cp.float64)
...
def doubleGpuGraphPolyFHT(reshapedX, radem, S, Z, int polydegree,
    ...
    if not radem.dtype == "int8":
        raise ValueError("radem must be int8.")
    if not Z.dtype == "float64" or not reshapedX.dtype == "float64":
        raise ValueError("reshapedX must be float64, Z must be float64.")
    if not S.dtype == "float64":
        raise ValueError("S must be float64.")
...
@cython.boundscheck(False)
@cython.wraparound(False)
def doubleGpuPolyFHT(reshapedX, radem, S, Z, int polydegree,
                int numThreads):
    ...
    cdef double *ZPtr = <double*>addr_Z
    ...
    Z[:] = reshapedX
    errCode = doubleCudaSORF3d(ZPtr, radem_ptr, Z.shape[0], Z.shape[1],
                Z.shape[2])
    if errCode.decode("UTF-8") != "no_error":
