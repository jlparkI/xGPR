...
class KernelBaseclass(ABC):
    ...
    Attributes:
        ...
        xmean (ndarray): Either None or an ndarray of shape (num_rffs) placed on the
            current device. If not None, this is subtracted from all features that
            are generated.
        xvar (ndarray): Either None or an ndarray of shape (num_rffs) placed on the
            current device. If not None, all features generated are divided by this.
            It is automatically clipped so the minimum value is >= 1e-14.
    """
    ...
    def set_xmean_xvar(self, xmean, xvar):
        """Sets the mean and variance using values supplied
        by caller. These values will then be used henceforth
        to adjust any features generated by this kernel. These
        are not properties because external classes should not
        be able to set these."""
        if xmean.shape[0] != self.num_rffs or \
                xvar.shape[0] != self.num_rffs:
            raise RuntimeError("The shape of the mean or variance "
                    "supplied to a kernel class was incorrect.")
        if self.device == "cuda":
            xmean = cp.asarray(xmean)
            xvar = cp.asarray(xvar)

        self.xmean = xmean
        self.xvar = xvar.clip(min=1e-14)
        if self.fit_intercept:
            self.xmean[0] = 0
            self.xvar[0] = 1


    def get_xmean_xvar(self):
        """Convenience function which returns the current xmean
        and xvariance if they have been set (otherwise these will
        be None)."""
        return self.xmean, self.xvar


    def reset_xmean_xvar(self):
        """Resets both xmean and xvar to be None."""
        self.xmean = None
        self.xvar = None
    ...
    def transform_x(self, input_x, sequence_length = None):
        ...
        if self.xmean is not None:
            xtrans -= self.xmean[None,:]
            xtrans /= self.xvar[None,:]
    ...
    def device(self, value):
        ...
        if self.xmean is not None:
            if value == "cpu":
                if not isinstance(self.xmean, np.ndarray):
                    self.xmean = cp.asnumpy(self.xmean)
                    self.xvar = cp.asnumpy(self.xvar)
            else:
                self.xmean = cp.asarray(self.xmean)
                self.xvar = cp.asarray(self.xvar)
